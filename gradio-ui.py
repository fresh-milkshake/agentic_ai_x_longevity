...
from src.constants.general import (
    PATENTS_DIR, RESULTS_INTERMEDIATE_DIR, RESULTS_FINAL_DIR, 
    USPT_API_KEY
)
...
def get_project_stats():
    stats = {
        "patents_count": len(list(PATENTS_DIR.glob("*.pdf"))) if PATENTS_DIR.exists() else 0,
        "processed_files": len(list(RESULTS_INTERMEDIATE_DIR.glob("*.pkl"))) if RESULTS_INTERMEDIATE_DIR.exists() else 0,
        "final_results": len(list(RESULTS_FINAL_DIR.glob("*.csv"))) if RESULTS_FINAL_DIR.exists() else 0,
    ...
    try:
        ...
        for i, patent in enumerate(patents):
            try:
                ...
                processing_status["message"] = f"Скачивание патента {i+1}/{len(patents)}: {patent.id}"
                
                patents_registry.download_document(
                    patent_id=patent.id,
                    path=PATENTS_DIR,
                    filename=f"US{patent.id}B2"
                )
                downloaded += 1
            except Exception as e:
    ...
    try:
        ...
        for i, pagedata in enumerate(results.interactions, 1):
            ...
            for j, interaction in enumerate(interactions, 1):
                ...
                param_str = []
                if params.Ki: param_str.append(f"Ki: {params.Ki}")
                if params.IC50: param_str.append(f"IC50: {params.IC50}")
                if params.Kd: param_str.append(f"Kd: {params.Kd}")
                if params.EC50: param_str.append(f"EC50: {params.EC50}")
...
def download_pdf_file(filename):
    ...
    try:
        ...
        return gr.File(value=str(file_path), visible=True), f"Файл {filename} готов к скачиванию"
    ...
    **Платформа для извлечения данных о связывании лигандов с белками из патентов**
    ...
    with gr.Tabs():
        with gr.TabItem("Обработка текста"):
            ...
            with gr.Row():
                file_input = gr.File(
                    label="Или загрузите текстовый файл",
                    file_types=[".txt", ".md"]
                )
                file_btn = gr.Button("Обработать файл")
            
            file_output = gr.Textbox(
                label="Результаты обработки файла",
                lines=10,
                interactive=False
            )
        ...
        with gr.TabItem("Управление патентами"):
            ...
            with gr.Row():
                ...
                with gr.Column():
                    pdf_download_file = gr.File(label="Скачанный PDF файл", visible=False)
                    pdf_download_status = gr.Markdown("Выберите PDF файл для скачивания")
            
            gr.Markdown("### Полная обработка")
            pipeline_btn = gr.Button("Запустить полный пайплайн", variant="secondary", size="lg")
            pipeline_output = gr.Textbox(
                label="Результат выполнения пайплайна",
                lines=5,
                interactive=False
            )
        ...
        with gr.TabItem("Результаты"):
            ...
            with gr.Row():
                ...
                with gr.Column(scale=2):
                    results_table = gr.Dataframe(
                        label="Данные о взаимодействиях",
                        interactive=False,
                        wrap=True
                    )
            ...
            with gr.Row():
                ...
                with gr.Column():
                    csv_download_file = gr.File(label="Скачанный CSV файл", visible=False)
                    csv_download_status = gr.Markdown("Выберите CSV файл для скачивания")
            
            gr.Markdown("### Экспорт объединенных результатов")
            
            with gr.Row():
                with gr.Column():
                    export_btn = gr.Button("Создать объединенный экспорт", variant="primary")
                    export_download_btn = gr.Button("Скачать объединенный файл", variant="secondary")
                
                with gr.Column():
                    export_download_file = gr.File(label="Объединенный CSV файл", visible=False)
    ...
    download_btn.click(
        download_patents,
        inputs=[patent_count, patent_query],
    ...
    refresh_stats_btn.click(
        lambda: format_stats_display(get_project_stats()),
    ...
    results_list.change(
        load_result_file,
        inputs=results_list,
        outputs=[results_table, result_info]
    )
    ...
    export_btn.click(
        lambda: export_combined_results()[1],
        outputs=export_output
    )
    
    export_download_btn.click(
        create_combined_export,
        outputs=[export_download_file, export_output]
    )
